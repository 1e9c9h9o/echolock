 ...
 */
export function calculateHealthStatus(lastHeartbeatTime: Date | string | null): HealthStatus {
 ...
 */
export async function queryGuardianHeartbeats(
  guardianNpubs: string[],
  relayUrls: string[] = DEFAULT_RELAYS
): Promise<Map<string, HeartbeatData>> {
  ...
  try {
    ...
    const events = await pool.querySync(relayUrls, filter) as NostrEvent[];

    // Process events and find latest for each guardian
    for (const event of events) {
      const existingHeartbeat = heartbeats.get(event.pubkey);
      if (!existingHeartbeat || event.created_at > existingHeartbeat.lastHeartbeat.getTime() / 1000) {
...
async function sendGuardianAlert(
  ...
  try {
    ...
    const recentAlertResult = await query<{ id: string }>(
    ...
    const settingsResult = await query<AlertSettings>(
    ...
    const settings: AlertSettings = settingsResult.rows[0] || {
    ...
    if (settings.webhook_url) {
      try {
        ...
        const webhookError = webhookErr as Error;
...
export async function runHealthCheck(): Promise<void> {
  ...
  try {
    ...
    const switchesResult = await query<SwitchRow>(
    ...
    const allGuardianNpubs = new Set<string>();
    const switchGuardianMap = new Map<string, SwitchGuardianData>();
 ...
 */
export function startGuardianHealthMonitor(): void {
  ...
  setTimeout(() => {
    runHealthCheck().catch(err => {
      logger.error('Initial guardian health check failed', { error: (err as Error).message });
    });
  }, 30000);
}

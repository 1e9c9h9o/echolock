 ...
 */
async function processRemindersForThreshold(thresholdHours: number): Promise<void> {
  try {
    ...
    const result = await query<SwitchReminderRow>(
      `SELECT
        s.id,
        s.user_id,
        s.title,
 ...
 */
async function sendReminder(sw: SwitchReminderRow, thresholdHours: number): Promise<void> {
 ...
 * Runs every hour
 */
export function startReminderMonitor(): ScheduledTask {
 ...
 */
export function stopReminderMonitor(job: ScheduledTask | null): void {
  if (job) {
    job.stop();

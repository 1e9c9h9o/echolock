 ...
 * @returns {Object|null} Transaction info if funded, null otherwise
 */
async function checkAddressFunded(address, network = 'testnet') {
 ...
 */
export async function checkPendingCommitments() {
  try {
 ...
 * @param {Object} commitment - Commitment data from database
 */
async function checkAndUpdateCommitment(commitment) {
 ...
 */
async function checkTransactionConfirmation(commitment) {
  const { id: switchId, user_id: userId, title, bitcoin_txid: txid, bitcoin_network: network } = commitment;
  const baseUrl = network === 'mainnet' ? BLOCKSTREAM_MAINNET : BLOCKSTREAM_TESTNET;
 ...
 */
export function startBitcoinFundingMonitor() {
 ...
 */
export function stopBitcoinFundingMonitor(job) {
  if (job) {
    job.stop();

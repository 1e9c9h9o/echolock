...
interface UpdatedSwitchRow {
  check_in_count: number;
  expires_at: Date;
}

/**
 * Custom error for transaction
 */
...
router.get('/:token/validate', async (req: Request, res: Response) => {
  try {
    ...
    const result = await query<CheckinTokenRow>(
...
router.post('/:token', async (req: Request, res: Response) => {
  try {
    ...
    const result = await transaction(async (client: PoolClient) => {
      ...
      const tokenResult = await client.query<CheckinTokenRow>(
      ...
      const updateResult = await client.query<UpdatedSwitchRow>(
        `UPDATE switches
    ...
    const error = err as (Error & { code?: string }) | TransactionError;

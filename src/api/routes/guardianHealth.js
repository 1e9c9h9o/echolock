 ...
 */
router.get('/:switchId/guardians/health', async (req, res) => {
  try {
    ...
    const guardiansWithHealth = guardians.map(guardian => {
 ...
 */
router.get('/:switchId/guardians/history', async (req, res) => {
  try {
    const userId = req.user.id;
    const { switchId } = req.params;
    const { hours = 168, guardianNpub } = req.query; // Default 7 days
...
router.put('/alert-settings', async (req, res) => {
  try {
    ...
    const validHours = Math.min(Math.max(1, parseInt(alertHoursBeforeCritical) || 24), 168);
 ...
 */
router.post('/:switchId/guardians/:guardianId/replace', async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    // Note: The actual share re-encryption would happen on the client side
    // The server just tracks the replacement for UI purposes
    guardians[guardianIndex] = {
      ...oldGuardian,
      npub: newGuardian.npub,
      name: newGuardian.name || oldGuardian.name,
      type: newGuardian.type || oldGuardian.type,
      status: 'pending', // New guardian needs to acknowledge
 ...
 */
router.get('/:switchId/guardians/alerts', async (req, res) => {
  try {
    const userId = req.user.id;
    const { switchId } = req.params;
 ...
 */
router.post('/:switchId/guardians/alerts/:alertId/acknowledge', async (req, res) => {
  try {
    const userId = req.user.id;

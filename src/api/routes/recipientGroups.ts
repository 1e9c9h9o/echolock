...
import { authenticateToken, checkRateLimit, AuthenticatedRequest } from '../middleware/auth.js';
...
interface ModifyRecipientsBody {
  recipientIds: string[];
}
...
router.get('/', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = await query<RecipientGroupRow>(
      `SELECT
        rg.id,
        rg.name,
        rg.description,
    ...
    res.json({
      ...
      data: {
        groups: result.rows.map(row => ({
          ...row,
          recipientCount: parseInt(row.recipient_count || '0') || 0
        })),
...
router.get('/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const groupResult = await query<RecipientGroupRow>(
...
router.patch('/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const groupId = req.params.id;
    const { name, description } = req.body as UpdateGroupBody;
...
router.post('/:id/recipients', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const groupId = req.params.id;
    const { recipientIds } = req.body as ModifyRecipientsBody;
 ...
 */
router.delete('/:id/recipients', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const groupId = req.params.id;
    const { recipientIds } = req.body as ModifyRecipientsBody;
 ...
 */
router.patch('/:groupId/recipients/:recipientId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const { groupId, recipientId } = req.params;
    const { customMessage } = req.body as UpdateRecipientBody;
 ...
 */
router.get('/recipients/read-receipts', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;

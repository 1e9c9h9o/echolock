...
interface SwitchWithUserRow extends SwitchRow {
  user_email: string;
}

interface FragmentMetadata {
  guardians?: Guardian[];
  bitcoinCommitment?: BitcoinCommitment;
}
...
interface HealthCheck {
  status: 'pass' | 'warning' | 'fail' | 'pending' | 'optional';
  message: string;
  [key: string]: unknown;
}
...
router.get('/:switchId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const proofResult = await query<ProofDocumentRow>(
 ...
 */
router.post('/:switchId/generate-proof', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const { switchId } = req.params;

    // Get switch and health data
    const switchResult = await query<SwitchWithUserRow>(
      `SELECT
        s.*,
 ...
 */
router.get('/:switchId/proof/:proofId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const { switchId, proofId } = req.params;

    // Verify ownership and get proof
    const result = await query<ProofDocumentRow>(

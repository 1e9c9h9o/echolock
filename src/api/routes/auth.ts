...
// Extend global for WebSocket tickets
declare global {
  // eslint-disable-next-line no-var
  var wsTickets: Map<string, { userId: string; email: string; expiresAt: number }> | undefined;
}
...
interface TwoFactorUserRow extends UserRow {
  totp_secret: string;
  backup_codes: string | BackupCode[];
...
const getCookieConfig = (maxAge: number): CookieOptions => {
...
router.post('/login', validateLogin, async (req: Request, res: Response) => {
  try {
    ...
    if (!isTestAccount(email)) {
      const twoFactorResult = await query<TwoFactorRow>(
...
router.post('/2fa/verify', async (req: Request, res: Response) => {
  try {
    const { challengeToken, code } = req.body as TwoFactorVerifyBody;
    ...
    const result = await query<TwoFactorUserRow>(
    ...
    // Decrypt TOTP secret
    const secret = decryptWithServiceKey(user.totp_secret).toString();
    ...
    if (!isValid && user.backup_codes) {
      const backupCodes: BackupCode[] = typeof user.backup_codes === 'string'
...
router.post('/refresh', async (req: Request, res: Response) => {
  try {
    ...
    const refreshTokenValue = req.cookies?.refreshToken || (req.body as RefreshBody).refreshToken;
 ...
 */
router.post('/resend-verification', validateEmail, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/reset-password', validatePassword, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/ws-ticket', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {

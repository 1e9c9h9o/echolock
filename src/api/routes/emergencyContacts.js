...
router.post('/', async (req, res) => {
  try {
    const userId = req.user.id;
    const { name, email, alertThresholdHours, escalationOrder } = req.body;
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    const { name, email, alertThresholdHours, escalationOrder, isActive } = req.body;
...
export const acknowledgeAlertRoute = express.Router();

/**
 * GET /api/acknowledge-alert/:token
 * Acknowledge an emergency alert via email link
 */
acknowledgeAlertRoute.get('/:token', async (req, res) => {
  try {

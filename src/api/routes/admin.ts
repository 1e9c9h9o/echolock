...
function requireMasterKey(req: Request, res: Response, next: NextFunction): void | Response {
  const masterKey = process.env.SERVICE_MASTER_KEY;
  const providedKey = req.headers['x-master-key'] as string | undefined;
 ...
 */
router.post('/manual-release/:switchId', requireMasterKey, async (req: Request, res: Response) => {
  const { switchId } = req.params;
  const clientIp = (req.headers['x-forwarded-for'] as string) || req.socket.remoteAddress || 'unknown';
  const forceRelease = (req.body as ManualReleaseBody).force === true && process.env.NODE_ENV !== 'production';
  ...
  try {
    ...
    if (sw.status === 'RELEASED' && sw.released_at) {
      const logResult = await query<ReleaseLogRow>(
    ...
    if (!message) {
      throw new Error('No message was reconstructed');
    }
    ...
    const existingLogs = await query<ReleaseLogRow>(
      'SELECT * FROM release_log WHERE switch_id = $1',
      [switchId]
    );
 ...
 */
router.get('/switch-status/:switchId', requireMasterKey, async (req: Request, res: Response) => {
  const { switchId } = req.params;
  ...
  try {
    ...
    // Get release logs
    const logs = await query<ReleaseLogRow>(

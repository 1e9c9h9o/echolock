 ...
 */
router.post('/:switchId', requireEmailVerified, async (req, res) => {
  try {
    const userId = req.user.id;
    const { switchId } = req.params;
    const {
 ...
 */
router.patch('/:switchId/:cascadeId', async (req, res) => {
  try {
    const userId = req.user.id;
    const { switchId, cascadeId } = req.params;
    const { delayHours, recipientGroupId, sortOrder } = req.body;
    ...
    const cascadeCheck = await query(
 ...
 */
router.delete('/:switchId/:cascadeId', async (req, res) => {
  try {
    const userId = req.user.id;
    const { switchId, cascadeId } = req.params;
 ...
 */
router.post('/:switchId/reorder', async (req, res) => {
  try {
    const userId = req.user.id;
    const { switchId } = req.params;

 ...
 */
async function sendEmergencyAlert(switchId, switchTitle, contactId, contactName, contactEmail, alertType, hoursRemaining) {
  try {
 ...
 */
export async function acknowledgeAlert(ackToken) {
  try {
    const result = await query(`
      UPDATE emergency_alerts
 ...
 */
export async function getPendingAlerts(switchId) {
  try {
    const result = await query(`
